export function getUSEntityData(): Object[] {
  const data: any[] = [];
  const entityData: string[] = [
    'Izon Itex Investment LLC',
    'Izon Itex Co LLC',
    'Izon Itex Mountains LLC',
    'Mountains Heart LLC',
    'John & Brothers LLC',
    'Free Mountains LLC',
    'Albert Sun LLC',
  ...
  for (let i: number = 0; i < 99; i++) {
    if (i == 1 || i === 8) {
      entityChild = [];
      for (let j: number = 0; j < entityData.length - 4; j++) {
        entityChild.push({
          'Entity/License Holder Name': entityData[j],
          'Domestic Jurisdiction': domesticData[j],
          'Entity Type': entityTypeData[j],
          'License #': '49951' + i + j,
          'License Type': licenseTypeData[j]
        });
      }
      data.push({
        'Entity/License Holder Name': entityData[entityCount],
        EntityChild: entityChild
      });
    } else {
      data.push({
        'Entity/License Holder Name': entityData[entityCount],
        'Domestic Jurisdiction': domesticData[entityCount],
        'Entity Type': entityTypeData[entityCount],
        'License #': '48951' + i,
        'License Type': licenseTypeData[entityCount]
      });
    }
    if (entityCount === entityData.length - 1) {
      entityCount = 0;
    } else {
      entityCount++;
    }
...
export function getNonUSEntityData(): Object[] {
  return [
    {
      'Entity Name': 'Izon Itex Co GmBH',
      Country: 'Germany',
      'Entity Type': 'limited liability company',
      'Jurisdiction Id': '23-23456',
      'Registered Date': '4/4/2020'
    }
  ];
}

export function getBusinessLicensesData(): Object[] {
  return [
    {
      'Entity/License Holder Name': 'Free Mountains LLC',
      'Domestic Jurisdiction': 'Texas',
      'License #': '67890',
      'License Type': 'Sales Tax',
      'License Authority': 'Department of Revenue'
    }
  ];
}
